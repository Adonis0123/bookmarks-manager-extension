...
export interface BookmarkTreeItem extends TreeItem<BookmarkNode> {
  index: TreeItemIndex;
  canMove: boolean;
  canRename: boolean;
  isFolder: boolean;
...
/**
 * 将 Chrome Bookmarks 节点转换为 React Complex Tree 所需的格式
 */
export const transformToTreeItems = (nodes: BookmarkNode[]): Record<TreeItemIndex, BookmarkTreeItem> => {
  const items: Record<TreeItemIndex, BookmarkTreeItem> = {};
  ...
  const traverse = (nodeList: BookmarkNode[]) => {
    nodeList.forEach(node => {
      ...
      items[node.id] = {
        index: node.id,
        canMove: !isRootFolder,
        canRename: !isRootFolder,
        isFolder,
        children: isFolder ? node.children!.map(child => child.id) : undefined,
        data: node,
      };
